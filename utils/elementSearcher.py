...
# Initializing
def initialize(_config: dict):
    global config
    config = _config

# Element searching
def findElement(key: str, searcher: WebElement | WebDriver):
    # Check if the key is in config
    if key not in config:
        raise Exception("The given key was not found in config! Key: " + key)

    # Get the search type and value
    searchType: By = typeStrToType(config[key]['type'])
    if searchType is None:
        raise Exception("The given type isn't found!")

    # Finally search for the element
    return searcher.find_element(searchType, config[key]['value'])
